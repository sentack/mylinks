...
export function DisplayCard({
  ...
  return (
    <div className="max-w-sm mx-auto p-6 bg-white dark:bg-gray-900 rounded-2xl shadow-lg text-center space-y-4">
      {/* Avatar */}
      {avatarUrl && (
      ...
      {/* Name */}
      <h1 className="text-2xl font-bold text-black dark:text-white">{fullName}</h1>

      {/* Bio */}
      {bio && <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed">{bio}</p>}
        ...
        <div className="flex justify-center gap-4 pt-4">
          {activeSocials.map(([platform, url]) => (
            <a
              ...
              className={`p-2 rounded-full ${accentColor} text-white hover:opacity-80 transition-opacity`}
            >
