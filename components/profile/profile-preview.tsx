...
interface ProfilePreviewProps {
  ...
  profileViewType: "classic" | "minimal" | "carded"
  accentColor: string
...
export function ProfilePreview({
  ...
  const bgClass = backgroundTheme === "dark" ? "bg-black text-white" : "bg-white text-black"
  const accentClass = accentColor

  const filledSocialLinks = Object.entries(socialLinks).filter(([, url]) => url && url.trim())

  if (profileViewType === "classic") {
    return (
      <div
        className={`${bgClass} rounded-2xl shadow-lg dark:shadow-md p-8 transition-all duration-300 hover:scale-[1.01]`}
      >
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          {fullName && <h2 className="text-3xl font-bold">{fullName}</h2>}

          {position && <p className="text-lg italic text-gray-600 dark:text-gray-400">{position}</p>}
          ...
          {bio && <p className="text-gray-700 dark:text-gray-300 max-w-sm">{bio}</p>}
        ...
        <div className="space-y-6">
          <div className="flex gap-6">
            ...
            <div className="flex-1">
              {fullName && <h2 className="text-2xl font-bold">{fullName}</h2>}
              {position && <p className="text-gray-600 dark:text-gray-400">{position}</p>}
            </div>
          </div>
  ...
  return (
    <div
      className={`${bgClass} rounded-2xl shadow-lg dark:shadow-md overflow-hidden transition-all duration-300 hover:scale-[1.01]`}
    >
      <div className={`h-24 ${accentClass}`} />
        ...
        <div className="flex gap-6">
          {profilePictureUrl && (
            <img
              src={profilePictureUrl || "/placeholder.svg"}
              alt={fullName}
              className="w-24 h-24 rounded-full object-cover border-4 border-white dark:border-black -mt-16"
            />
          )}
          <div className="flex-1 pt-4">
            {fullName && <h2 className="text-2xl font-bold">{fullName}</h2>}
            {position && <p className="text-gray-600 dark:text-gray-400 italic">{position}</p>}
          </div>
        </div>

        {companyName && (
          <div className={`p-4 rounded-lg ${backgroundTheme === "dark" ? "bg-gray-900" : "bg-gray-100"}`}>
            <p className="font-medium">{companyName}</p>
            {companyWebsite && (
              <a
                href={companyWebsite}
